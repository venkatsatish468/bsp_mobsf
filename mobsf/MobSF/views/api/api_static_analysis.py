...
from mobsf.StaticAnalyzer.views.android.static_analyzer import minimalistic_static_analyzer
...
def api_scan(request):
    print("Performing Full Scan and Analysis")
    """POST - Scan API."""
...
@request_method(['POST'])
@csrf_exempt
def api_minimalistic_scan(request):
    print("Performing Minimalistic Scan")
    """POST - Scan API."""
    params = {'scan_type', 'hash', 'file_name'}
    if set(request.POST) < params:
        return make_api_response(
    ...
    # APK, Android ZIP and iOS ZIP
    if scan_type in {'xapk', 'apk', 'zip'}:
        resp = minimalistic_static_analyzer(request, True)
        if 'type' in resp:
            # For now it's only ios_zip
            request.POST._mutable = True
            request.POST['scan_type'] = 'ios'
