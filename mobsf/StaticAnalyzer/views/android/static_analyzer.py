            ...
            if typ == 'apk':
                ...
                if db_entry.exists() and not rescan:
                    ...
                    code_an_dic = code_analysis(
                        app_dic['app_dir'],
                        'apk',
                        app_dic['manifest_file'],app_dic['md5'])

                    logger.info("CODEEEEE",code_an_dic)
...
######andoird minimalistic scan - written for Zebra - BSP Scanner 

def minimalistic_static_analyzer(request, api=False):
    """Do static analysis on an request and save to db."""
    def default_value():
        return ""
    try:
        ...
        if re_scan == '1':
            rescan = True
        # Input validation
        app_dic = defaultdict(default_value)
        match = re.match('^[0-9a-f]{32}$', checksum)
        if (match
                and filename.lower().endswith(
                    ('.apk', '.xapk', '.zip', '.apks'))
            ...
            if typ == 'apk':
                ...
                if db_entry.exists() and not rescan:
                    ...
                    elf_dict = defaultdict(default_value)
                    cert_dic = cert_info(
                        app_dic['app_dir'],
                        app_dic['app_file'])
                    code_an_dic=defaultdict(default_value)
                    man_an_dic=defaultdict(default_value)
                    apkid_results=""
                    quark_results=""
                    tracker_res=""
                    # apkid_results = apkid_analysis(app_dic[
