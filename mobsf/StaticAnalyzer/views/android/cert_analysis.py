...
def cert_info(app_dir, app_file):
    ...
    try:
        ...
        apkSignerOutput = a.get_md5_sha1_sha256()
        hashfunctions = {
            'md5': hashlib.md5,
            'sha1': hashlib.sha1,
        ...
        certlist.append('md5: {}'.format(apkSignerOutput["md5"]))
        certlist.append('sha1: {}'.format(apkSignerOutput["sha1"]))
        certlist.append('sha256: {}'.format(apkSignerOutput["sha256"]))
        ...
        for cert in certs:
            ...
            # Removing the below two line as we were getting errors
        ...
        # My Modification
        certOutputSplitList = cert_dic['certificate_info'].splitlines()
        result = {}
        for line in certOutputSplitList:
            lineSplitList = line.split(":")
            if len(lineSplitList) == 2:
                result[lineSplitList[0].strip()] = lineSplitList[1].strip()
        cert_dic["v1"] = result["v1 signature"]
        cert_dic["v2"] = result['v2 signature']
        cert_dic["v3"] = result['v3 signature']
        cert_dic["subject"] = result["Subject"]
        cert_dic["sha1"] = result["sha1"]
        cert_dic["sha256"] = result["sha256"]
        subjectStr = cert_dic["subject"]
        subjectStrSplitList = subjectStr.split(",")
        newSubjectDict = {}
        for splitStr in subjectStrSplitList:
            splitEqual = splitStr.split("=")
            if len(splitEqual) == 2:
                newSubjectDict[splitEqual[0].strip()] = splitEqual[1].strip()
        # pprint(newSubjectDict)
        cert_dic["O"] = newSubjectDict["O"]
        cert_dic["CN"] = newSubjectDict["CN"]
        cert_dic["OU"] = newSubjectDict["OU"]        
        return cert_dic
    except Exception:
